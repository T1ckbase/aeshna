...
fn getCaptureRect(filter: *const AeshnaFilterData, source_width: u32, source_height: u32) ?CaptureRect {
    if (source_width == 0 or source_height == 0) return null;

    const capture_width = @min(if (filter.width == 0) source_width else filter.width, source_width);
    const capture_height = @min(if (filter.height == 0) source_height else filter.height, source_height);

    if (capture_width == 0 or capture_height == 0) return null;

    const centered_x = (source_width - capture_width) / 2;
    const centered_y = (source_height - capture_height) / 2;

    return .{
        .x = if (filter.offset_enabled) clampCaptureStart(centered_x, source_width - capture_width, filter.offset_x) else centered_x,
        .y = if (filter.offset_enabled) clampCaptureStart(centered_y, source_height - capture_height, filter.offset_y) else centered_y,
        .width = capture_width,
        .height = capture_height,
    };
}

fn ensureStageSurface(filter: *AeshnaFilterData, source_width: u32, source_height: u32) bool {
    if (filter.stagesurface != null and filter.stage_width == source_width and filter.stage_height == source_height) {
        return true;
    }

    if (filter.stagesurface) |stagesurface| {
        obs.gs_stagesurface_destroy(stagesurface);
        filter.stagesurface = null;
    }

    filter.stagesurface = obs.gs_stagesurface_create(source_width, source_height, obs.GS_BGRA) orelse {
        module.log.err("failed to create stage surface for {}x{}", .{ source_width, source_height });
        filter.stage_width = 0;
        filter.stage_height = 0;
        return false;
    };
    filter.stage_width = source_width;
    filter.stage_height = source_height;
    return true;
}

fn ensureCaptureBuffer(filter: *AeshnaFilterData, capture_width: u32, capture_height: u32) bool {
    ...
    filter.capture_buffer.resize(capture_allocator, total_bytes) catch {
        module.log.err("failed to resize capture buffer to {} bytes", .{total_bytes});
        return false;
    };

    filter.capture_stride = @intCast(stride);
    filter.captured_width = capture_width;
    filter.captured_height = capture_height;
    return true;
}

fn pixelMatchesMask(filter: *const AeshnaFilterData, pixel: []const u8) bool {
    const mask_color: u32 = @intCast(filter.mask_color);
...
fn copyCaptureRegion(filter: *AeshnaFilterData, capture_rect: CaptureRect, data: [*c]u8, linesize: u32) bool {
    if (!ensureCaptureBuffer(filter, capture_rect.width, capture_rect.height)) return false;
    ...
    const dest_stride: usize = @intCast(filter.capture_stride);
    ...
    while (row < capture_height) : (row += 1) {
        ...
        const dst = filter.capture_buffer.items[dst_offset .. dst_offset + row_bytes];
        @memcpy(dst, src);
    }
...
fn captureContainsTriggerColor(filter: *const AeshnaFilterData) bool {
    const stride: usize = @intCast(filter.capture_stride);
    const width: usize = @intCast(filter.captured_width);
    const height: usize = @intCast(filter.captured_height);
    const row_bytes = width * bgra_bytes_per_pixel;

    var row: usize = 0;
    while (row < height) : (row += 1) {
        const row_start = row * stride;
        const pixels = filter.capture_buffer.items[row_start .. row_start + row_bytes];

        var pixel_offset: usize = 0;
        while (pixel_offset < row_bytes) : (pixel_offset += bgra_bytes_per_pixel) {
            if (!pixelMatchesMask(filter, pixels[pixel_offset .. pixel_offset + bgra_bytes_per_pixel])) {
                return true;
            }
        }
    }
...
fn tryTrigger(filter: *AeshnaFilterData, found_trigger_pixel: bool) void {
    ...
    const throttle_ns = @as(u64, filter.throttle_ms) * std.time.ns_per_ms;
    if (throttle_ns != 0 and filter.last_trigger_at_ns != 0 and now - filter.last_trigger_at_ns < throttle_ns) {
        return;
    }

    filter.last_trigger_at_ns = now;
    // TODO: synthesize the click/input event here.
}

fn renderTargetTexture(filter: *AeshnaFilterData, target: *obs.obs_source_t, parent: *obs.obs_source_t, source_width: u32, source_height: u32) ?*obs.gs_texture_t {
    obs.gs_texrender_reset(filter.texrender);

    obs.gs_blend_state_push();
    defer obs.gs_blend_state_pop();
    obs.gs_blend_function(obs.GS_BLEND_ONE, obs.GS_BLEND_ZERO);

    if (!obs.gs_texrender_begin(filter.texrender, source_width, source_height)) return null;
    ...
    return obs.gs_texrender_get_texture(filter.texrender);
}
...
fn tryRegisterHotkeys(filter: *AeshnaFilterData) void {
    if (filter.hotkeys_loaded) return;
    ...
    filter.hotkey_active = hotkey_id;
    filter.hotkeys_loaded = true;
}
...
fn hotkey_active(data: ?*anyopaque, _: obs.obs_hotkey_id, _: ?*obs.obs_hotkey_t, pressed: bool) callconv(.c) void {
    ...
    filter.active.store(pressed, .seq_cst);
    // module.log.info("Status: {s}\n", .{if (pressed) "down" else "up"});
}

fn aeshna_filter_get_name(_: ?*anyopaque) callconv(.c) [*c]const u8 {
    return "Aeshna"; // or module.text()
}
...
fn aeshna_filter_create(setting: ?*obs.obs_data_t, context: ?*obs.obs_source_t) callconv(.c) ?*anyopaque {
    ...
    const texrender = obs.gs_texrender_create(obs.GS_BGRA, obs.GS_ZS_NONE) orelse {
        obs.bfree(ptr);
        module.log.err("Failed to create render texrenderer", .{});
        return null;
    };
...
fn aeshna_filter_destroy(data: ?*anyopaque) callconv(.c) void {
    ...
    if (filter.hotkey_active != obs.OBS_INVALID_HOTKEY_ID) {
        obs.obs_hotkey_unregister(filter.hotkey_active);
    }
    ...
    filter.capture_buffer.deinit(capture_allocator);
    obs.bfree(ptr);
}
...
fn aeshna_filter_update(data: ?*anyopaque, settings: ?*obs.obs_data_t) callconv(.c) void {
    ...
    filter.enabled = obs.obs_data_get_bool(obs_settings, Setting.ENABLED);
    filter.mask_color = obs.obs_data_get_int(obs_settings, Setting.MASK_COLOR);
    const throttle_value = obs.obs_data_get_int(obs_settings, Setting.THROTTLE_MS);
    filter.throttle_ms = @intCast(if (throttle_value > 0) throttle_value else 0);
    filter.offset_enabled = obs.obs_data_get_bool(obs_settings, Setting.OFFSET_ENABLED);
    filter.offset_x = @intCast(obs.obs_data_get_int(obs_settings, Setting.OFFSET_X));
    filter.offset_y = @intCast(obs.obs_data_get_int(obs_settings, Setting.OFFSET_Y));

    const requested_width = obs.obs_data_get_int(obs_settings, Setting.CAPTURE_WIDTH);
    const requested_height = obs.obs_data_get_int(obs_settings, Setting.CAPTURE_HEIGHT);
    filter.width = @intCast(if (requested_width > 0) requested_width else 1);
    filter.height = @intCast(if (requested_height > 0) requested_height else 1);
}

fn aeshna_filter_defaults(settings: ?*obs.obs_data_t) callconv(.c) void {
    const obs_settings = settings orelse return;

    obs.obs_data_set_default_bool(obs_settings, Setting.ENABLED, true);
    obs.obs_data_set_default_int(obs_settings, Setting.MASK_COLOR, 0x0);
    obs.obs_data_set_default_int(obs_settings, Setting.THROTTLE_MS, 100);
    obs.obs_data_set_default_bool(obs_settings, Setting.OFFSET_ENABLED, false);
    obs.obs_data_set_default_int(obs_settings, Setting.OFFSET_X, 0);
    obs.obs_data_set_default_int(obs_settings, Setting.OFFSET_Y, 0);
    obs.obs_data_set_default_int(obs_settings, Setting.CAPTURE_WIDTH, 6);
    obs.obs_data_set_default_int(obs_settings, Setting.CAPTURE_HEIGHT, 6);
}
...
fn aeshna_filter_render(data: ?*anyopaque, effect: ?*obs.gs_effect_t) callconv(.c) void {
    ...
    const active = filter.active.load(.seq_cst);
    if (!filter.enabled or !active) {
        obs.obs_source_skip_video_filter(filter.context);
        return;
    }
    ...
    const texture = renderTargetTexture(filter, target, parent, source_width, source_height) orelse {
        obs.obs_source_skip_video_filter(filter.context);
        return;
    };

    if (ensureStageSurface(filter, source_width, source_height)) {
        obs.gs_stage_texture(filter.stagesurface.?, texture);

        if (getCaptureRect(filter, source_width, source_height)) |capture_rect| {
            var mapped_data: [*c]u8 = null;
            var mapped_linesize: u32 = 0;

            if (obs.gs_stagesurface_map(filter.stagesurface.?, &mapped_data, &mapped_linesize)) {
                const copied = copyCaptureRegion(filter, capture_rect, mapped_data, mapped_linesize);
                obs.gs_stagesurface_unmap(filter.stagesurface.?);

                if (copied) {
                    tryTrigger(filter, captureContainsTriggerColor(filter));
                }
            }
        }
    }
